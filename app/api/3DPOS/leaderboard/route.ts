...
async function getMonthlyLeaderboard(session: string) {
  const jobsUrl =
    "https://cloud.3dprinteros.com/apiglobal/get_finished_jobs_report";
  ...
  const jobCountByUser: {
    [email: string]: { firstname: string; lastname: string; count: number };
  } = {};
  ...
  const leaderboard = Object.entries(jobCountByUser)
    .map(([email, { firstname, lastname, count }]) => ({
      email,
      firstname,
      lastname,
      count,
