  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const response = await fetch(`/api/3DPOS/jobs?period=day&date=${today}`, {
          headers: {
            'x-printer-session': '8'
          }
        });
      ...
      <Card className="shadow-lg">
        <CardHeader className="pb-2">
          <CardTitle className="flex justify-between items-center">
            ...
            <Badge variant={
              parseInt(currentMetric.percentSuccessful) > 80 ? "default" :
              parseInt(currentMetric.percentSuccessful) > 60 ? "secondary" : "destructive"
            }>
